...
export function intersectRayBox(box: BoxType, origins: FloatArray, dirsInv: FloatArray, signs: Uint8Array, near: number, far: number): boolean {
    const xSign = signs[0];
    const ySign = signs[1];
    const zSign = signs[2];
...
export function intersectBoxBox(A: BoxType, B: BoxType): boolean {
    return A[1] >= B[0] && B[1] >= A[0] && A[3] >= B[2] && B[3] >= A[2] && A[5] >= B[4] && B[5] >= A[4];
}

export function intersectSphereBox(center: FloatArray, radius: number, box: BoxType): boolean {
